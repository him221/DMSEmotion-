'''Train Fer2013 with PyTorch.(PyTorch框架对FER2013数据集的模型训练，迭代轮次暂设定为250次'''
...
from models import *
import torch.optim as optim  # 添加此行
...
# 如果你想直接在代码中设置恢复训练：
opt.resume = True  # 将此行设置为True以启用恢复训练
...
#
#     # 加载 PublicTest 的检查点
#     checkpoint = torch.load(os.path.join(path, 'PublicTest_model.t7'))
#     net.load_state_dict(checkpoint['net'])  # 恢复网络参数
#     best_PublicTest_acc = checkpoint['best_PublicTest_acc']  # 恢复最好的公测准确率
#     best_PublicTest_acc_epoch = checkpoint['best_PublicTest_acc_epoch']  # 恢复最好的epoch
#     start_epoch = checkpoint['epoch'] + 1  # 恢复开始的 epoch
#
#     # 加载 PrivateTest 的检查点
#     checkpoint_private = torch.load(os.path.join(path, 'PrivateTest_model.t7'))
#     best_PrivateTest_acc = checkpoint_private['best_PrivateTest_acc']  # 恢复最好的私测准确率
#     best_PrivateTest_acc_epoch = checkpoint_private['best_PrivateTest_acc_epoch']  # 恢复私测准确率的epoch
# else:
#     print('==> Building model..')
